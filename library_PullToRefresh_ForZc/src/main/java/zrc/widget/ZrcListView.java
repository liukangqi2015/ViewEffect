...
public class ZrcListView extends ZrcAbsListView
{
	...
	public boolean requestChildRectangleOnScreen(View child, Rect rect, boolean immediate)
	{
		...
		} else if (rect.top < listUnfadedTop && rect.bottom < listUnfadedBottom)
		{
			...
			if (rect.height() > height)
			{
				// screen size chunk 屏幕尺寸
				scrollYDelta -= (listUnfadedBottom - rect.bottom);
			} else
			{
				// entire rect at top 整个屏幕的顶部
	...
	final int measureHeightOfChildren(int widthMeasureSpec, int startPosition, int endPosition, final int maxHeight, int disallowPartialChildPosition)
	{
		...
		for (i = startPosition; i <= endPosition; ++i)
		{
			...
			if (i > 0)
			{
				// Count the divider for all but one child：计数分压器的所有，但一个孩子
				returnedHeight += dividerHeight;
			}

			// Recycle the view before we possibly return from the method:回收的观点，我们可能从方法返回
			...
			{
				// We went over, figure out which height to return. If:我们走了，弄清楚哪个高度要回来
		...
		//在这一点上，我们经历了一系列的儿童，他们的每一个完全适合，所以返回返回的高度
		...
		try
		{
			...
			if (mAdapter != null && mItemCount != mAdapter.getCount())
			{
				throw new IllegalStateException("The content of the adapter has changed but "
						...
						+ "the UI thread. Make sure your adapter calls notifyDataSetChanged() " + "when its content changes. [in ListView(" + getId()
	...
	private View makeAndAddView(int position, int y, boolean flow, int childrenLeft, boolean selected)
	{
		...
		if (!mDataChanged)
		{
			// Try to use an existing view for this position：尝试使用现有的视图来进行这个位置
		...
		// possible
		//为这个位置做一个新的视图，或者转换一个未使用的视图,可能的


		child = obtainView(position, mIsScrap);

		// This needs to be positioned and measured:这需要被定位和测量
	...
	private void setupChild(View child, int position, int y, boolean flowDown, int childrenLeft, boolean selected, boolean recycled)
	{
		...
		ZrcAbsListView.LayoutParams p = null;
		if (child instanceof ZrcAbsListView)
		{
			p = (ZrcAbsListView.LayoutParams) child.getLayoutParams();
		}
		if (p == null)
		{
			p = (ZrcAbsListView.LayoutParams) generateDefaultLayoutParams();
		...
		if ((recycled && !p.forceAdd) || (p.recycledHeaderFooter && p.viewType == ZrcAdapterView.ITEM_VIEW_TYPE_HEADER_OR_FOOTER))
		{
	...
	public boolean isOpaque()
	{
		...
		if (retValue)
		{
			...
			final int listBottom = getHeight() - (mListPadding != null ? mListPadding.bottom + mLastBottomOffset : getPaddingBottom());
					...
					if (drawDividers && (bottom < listBottom))
					{
						...
						if (areAllItemsSelectable
								|| ((adapter.isEnabled(itemIndex) || (headerDividers && isHeader) || (footerDividers && isFooter)) && (isLastItem
										|| adapter.isEnabled(nextIndex) || (headerDividers && (nextIndex < headerCount)) || (footerDividers && (nextIndex >= footerLimit)))))
						{
							bounds.top = bottom;
