...
public abstract class ZrcAbsListView extends ZrcAdapterView<ListAdapter> implements ViewTreeObserver.OnTouchModeChangeListener
{
	...
	private boolean isDefaultAble = true;
	...
	public ZrcAbsListView(Context context, AttributeSet attrs, int defStyle)
	{
		...
		DefaultHeadable(context);
		DefaultFootable(context);
	...
	// 设置默认下拉刷新的样式（可选，但如果没有Header则无法下拉刷新）
	private void DefaultHeadable(Context context)
	{
		if (isDefaultAble)
		{
	...
	// 设置默认加载更多的样式（可选）
	private void DefaultFootable(Context context)
	{
		if (isDefaultAble)
		{
			SimpleHeader header = new SimpleHeader(context);
			header.setTextColor(0xff727272);
			header.setCircleColor(0xff727272);
			mZrcHeader = header;
		}
	}

	public void isDefaultAble(boolean b)
	{
		isDefaultAble = b;
	}
	...
	protected int computeVerticalScrollOffset()
	{
		...
		if (firstPosition >= 0 && childCount > 0)
		{
			if (mSmoothScrollbarEnabled)
			{
				...
				if (height > 0)
				{
					return Math.max((firstPosition * range - top * range / height) / count + mFirstTopOffset * 10 + mScrollY * 10,
							mFirstTopOffset * 10);
				}
			} else
			{
	...
	protected void dispatchDraw(Canvas canvas)
	{
		...
		if (zrcHeader != null)
		{
			...
			if (needShowHeader)
			{
				boolean more = zrcHeader.draw(canvas, listPadding.left, viewTop + mFirstTopOffset, getWidth() - listPadding.right, firstTop);
		...
		if (childCount > 0 && (isRefreshing || isLoadingMore) && footer != null)
		{
			...
			if (firstPosition + childCount == mItemCount)
			{
				...
				if (lastBottom < viewBottom)
				{
					...
					if (more)
					{
						postInvalidate(listPadding.left, listPadding.top, getWidth() - listPadding.right, viewBottom - mLastBottomOffset);
					}
				}
			}
		}
	...
	boolean performLongPress(final View child, final int longPressPosition, final long longPressId)
	{
		...
		{
			handled = mOnItemLongClickListener.onItemLongClick((ZrcListView) this, child, longPressPosition, longPressId);
		}
		if (handled)
		{
	...
	private void setRefreshDone(String msg, int state)
	{
		...
		if (zrcHeader != null && zrcHeader.getState() == Headable.STATE_LOADING)
		{
			...
			final int childCount = getChildCount();
			final int firstTop = childCount == 0 ? mFirstTop : getChildAt(0).getTop();
			final boolean unShownState = mFirstPosition > 0 || firstTop < mListPadding.top + mFirstTopOffset + zrcHeader.getHeight();
	...
	static class SavedState extends BaseSavedState
	{
		...
		{
			return "AbsListView.SavedState{" + Integer.toHexString(System.identityHashCode(this)) + " firstPosition=" + mFirstPosition
			/* + " firstTop=" + mFirstTop */+ "}";
		}
	}
	...
	private class FlingRunnable implements Runnable
	{
		...
		void endFling()
		{
			...
			if (mZrcHeader != null && oldTouchMode == TOUCH_MODE_RESCROLL && mZrcHeader.getState() == Headable.STATE_RELEASE)
			{
				scrollToAdjustViewsUpOrDown();
			}
		}
		...
		boolean scrollToAdjustViewsUpOrDown()
		{
			...
			final boolean isTooShort = childCount == mItemCount && lastBottom - firstTop < getHeight();
			final int bottomOffset = isTooShort ? firstTop - listPadding.top - mFirstTopOffset : lastBottom - getHeight() + listPadding.bottom
					+ mLastBottomOffset;
